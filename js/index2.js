...
$(function (){
   //清空按钮
    ...
    var url = urlPre + '/queryADResultBySource.do';
    //var url = './json/data2.json';
    var exportUrl = urlPre + '/lcwa/exportADResultBySource.do';
        ...
        $.ajax({
            ...
            success:function (data){
                ...
                $.each(data,function (index,cur){
                    ...
                    str += '<td class="rol1">'+ (typeof (cur.source) == 'undefined'?'无':cur.source) +'</td>';
                    str += '<td class="rol1">'+ (typeof (cur.zhuce) == 'undefined'?'无':cur.zhuce) +'</td>';
                    str += '<td class="rol1">'+ (typeof (cur.liuzi) == 'undefined'?'无':cur.liuzi) +'</td>';
                    str += '<td class="rol1">'+ (typeof (cur.huru) == 'undefined'?'无':cur.huru) +'</td>';
